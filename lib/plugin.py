...
def add_top_stories_menu_item():
    ...
    output = get_string(30012).format(pgm_title, pgm_description)
...
def add_live_schedule_menu_item():
    ...
    title = u'{0}-{1}: {2}'.format(broadcast_start_local.strftime('%H:%M'),
                                   ...
                                   row['title'])
    output = '{0}\n\n{1}'.format(
        get_string(30022).format(title), row['description'])
...
def vod_index():
    ...
    xbmcplugin.addDirectoryItem(
        ...
        xbmcgui.ListItem(get_string(30040), iconImage=NHK_ICON), True)
    xbmcplugin.addDirectoryItem(
        plugin.handle, plugin.url_for(vod_categories),
        xbmcgui.ListItem(get_string(30041), iconImage=NHK_ICON), True)
    xbmcplugin.addDirectoryItem(
        plugin.handle, plugin.url_for(vod_playlists),
        xbmcgui.ListItem(get_string(30042), iconImage=NHK_ICON), True)
    xbmcplugin.addDirectoryItem(
        plugin.handle,
        plugin.url_for(vod_episode_list, rest_url['get_latest_episodes'], 0, 0,
                       xbmcplugin.SORT_METHOD_DATEADDED),
        xbmcgui.ListItem(get_string(30043), iconImage=NHK_ICON), True)
    xbmcplugin.addDirectoryItem(
        plugin.handle,
        plugin.url_for(vod_episode_list, rest_url['get_most_watched_episodes'],
                       0, 0, xbmcplugin.SORT_METHOD_NONE),
        xbmcgui.ListItem(get_string(30044), iconImage=NHK_ICON), True)
    xbmcplugin.addDirectoryItem(
        plugin.handle,
        plugin.url_for(vod_episode_list, rest_url['get_all_episodes'], 0, 0,
                       xbmcplugin.SORT_METHOD_TITLE),
        xbmcgui.ListItem(get_string(30045), iconImage=NHK_ICON), True)
...
def vod_categories():
    ...
    for row in api_result_json['vod_categories']:
        ...
        api_url = rest_url['get_categories_episode_list'].format(categoryId)
        if (total_epsiodes > 0):
            li = xbmcgui.ListItem(title)
...
def vod_playlists():
    ...
    for row in api_result_json['data']['playlist']:
        ...
        api_url = rest_url['get_playlists_episode_list'].format(playlistId)
...
def live_schedule_index():
    ...
    for row in program_json:
        ...
        if (playable):
            title = get_string(30070).format(episode_name)
        else:
