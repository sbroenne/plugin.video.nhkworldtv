...
def test_request_url():
    assert url.request_url("https://www3.nhk.or.jp/nhkworld/", False).status_code == 200


def test_get_url():
    assert url.get_url("https://www3.nhk.or.jp/nhkworld/", False).status_code == 200
...
def test_request_url_notfound():
    assert url.get_url("https://www3.nhk.or.jp/doesnotexist/", False).status_code == 404
...
def test_get_JSON_non_cached():
    assert isinstance(url.get_json(nhk_api.rest_url["get_livestream"], False), dict)
